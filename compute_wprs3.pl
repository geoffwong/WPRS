...
sub comp_total
{
    ...
    # Insert Pilot Placing (Pp) Factors 
    # Pp = (score-last_pilot_score)/(winner_pilot_score-last_pilot_score)
    ...
    $sth = $dbh->prepare("insert into tblPilotFactors (cfPk, pilPk, cfPp) select cfPk, pilPk, ((CR.cmpScore - ?) / (? - ?) + (? - CR.cmpPos) / (? - 1))/2 from tblCompResult CR, tblCompFactors CF where CR.comPk=CF.comPk and CR.comPk=? and CF.cfPk=?");
