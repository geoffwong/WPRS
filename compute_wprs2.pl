...
sub comp_total
{
    ...
    $sth = $dbh->prepare("insert into tblPilotFactors (cfPk, pilPk, cfPp) select cfPk, pilPk, (CR.cmpScore - ?) / (? - ?) from tblCompResult CR, tblCompFactors CF where CR.comPk=CF.comPk and CR.comPk=? and CF.cfPk=?");
