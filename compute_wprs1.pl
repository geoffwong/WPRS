...
sub compute_wprs
{
    ...
    $sth = $dbh->prepare("select *, datediff(?,C.comDateTo) as DaysSinceEnd from tblCompetition C, tblCompFactors CF where C.comPk=CF.comPk and CF.cfName='$formula' and C.comDateTo between DATE_SUB(?, INTERVAL 3 YEAR) and ? group by C.comPk");
    $sth->execute($dateTo, $dateTo, $dateTo);
    ...
    #$dbh->do("insert into tblLadder (ladName, ladDateTo) values ('WPRS1', '$dateTo')");
...
sub comp_total
{
    ...
    $sth = $dbh->prepare("insert into tblPilotFactors (cfPk, pilPk, cfPp) select cfPk, pilPk, ((? - CR.cmpPos) / ?) from tblCompResult CR, tblCompFactors CF where CR.comPk=CF.comPk and CR.comPk=? and CF.cfPk=?");
...
# Create a starting ladding ..
# insert into tblLadder (ladName, ladDateTo) values ('WPRS2', '2001-01-01');
# insert into tblLadderResult (ladPk, pilPk, ldrPosition, ldrPoints) select 1627, pilPk, ldrPosition, ldrPoints from tblLadderResult where ladPk=1;
...
for $ref (@allcomps)
{
    ...
    $sth = $dbh->prepare("select * from tblLadder L, tblLadderResult LR where L.ladDateTo=? and L.LadName=? and LR.ladPk=L.ladPk");
    $sth->execute($lastWPRS, $formula);
    if ($sth->rows == 0)
    {
